...
[[new_aln]]*ALN* - align the origin counter::

Advances the origin counter to the specified boundary.
It's equivalent to `ORG [*+b-1]&~[b-1]` where `b` is the `ALN` operand.
+
Example:
+
---------------
 aln $400
font ins 'panther.fnt'
---------------
...
* 16-bit words: `a(10000)`
* low bytes of words: `l(511)` (byte 255)
* high bytes of words: `h(511)` (byte 1)
* 24-bit integers: `e($123456)`
* 32-bit integers: `f($12345678)`
...
Version 3.3.0 (2026-09-??)
~~~~~~~~~~~~~~~~~~~~~~~~~~
- built with LDC - supports macOS 15.4+
- <<new_aln,`ALN` directive>> for aligning the origin counter to the specified boundary
  (https://github.com/pfusik/xasm/issues/20[requested] by Sebastian Igielski)
- <<new_dta,`DTA e(int24bit),f(int32bit)` modes>>
  (compatible with Fast Assembler and MADS)
- avoid misinterpreting certain non-ASCII characters as Atari $9b EOL,
  by detecting EOL in the first line of each source file
  (https://github.com/pfusik/xasm/issues/21[reported] by Sebastian Igielski)
- *xasm* can now be used as a link:libxasm.html[library] in any D program
  (by Adrian Matoga)
- fixed writing the */t* label table: it no longer closes standard output
  when the listing goes to `-`, nor overwrites the listing file when both
  */l* and */t* name the same file (by Adrian Matoga)
